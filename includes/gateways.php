    ...
    switch ($digit_gateway) {
        ...
        case 16:
            ...
            $sms = $api->sms();
                ...
                // دیکشنری خطاها برای خدماتی و عادی
                $error_messages = [
                    // خطاهای عمومی (ارسال عادی و بین‌الملل)
                    '0' => 'نام کاربری یا رمز عبور اشتباه است.',
                    '2' => 'اعتبار کافی نیست.',
                    '3' => 'محدودیت در ارسال روزانه.',
                    '4' => 'شماره فرستنده معتبر نیست.',
                    '5' => 'سامانه در حال بروزرسانی است.',
                    ...
                    // خطاهای خدماتی اشتراکی
                    '-10' => 'لینک در متغیرهای ارسالی وجود دارد.',
                    '-7' => 'خطایی در شماره فرستنده رخ داده است.',
                    '-6' => 'خطای داخلی رخ داده است.',
                    '-5' => 'متن با متغیرهای پیش‌فرض همخوانی ندارد.',
                    '-4' => 'کد متن ارسالی صحیح نیست یا تأیید نشده است.',
                    '-3' => 'خط ارسالی در سیستم تعریف نشده است.',
                    '-2' => 'محدودیت تعداد شماره (هر بار یک شماره).',
                ...
                try {
                    if ($international == "1") {
                        if ($countrycode == "98") {
                            if ($khadamati == "1") {
                                $result = $sms->BaseServiceNumber($text, $to, $bodyId);
                            } elseif ($khadamati == "0") {
                                $result = $sms->send($to, $from, $text);
                            }
                        } else {
                            $result = $sms->send($to, "+SHARED", $international_text);
                        }
                    } else {
                        if ($khadamati == "1") {
                            $result = $sms->BaseServiceNumber($text, $to, $bodyId);
            ...
            if ($testCall) {
                        ...
                        if (isset($error_messages[$value])) {
                            $message = "" . $error_messages[$value] . " (Value: $value)";
                        } else {
                            $message = "Value = $value ($strRetStatus)";
                        }
                        return $message;
                    }
            
                    return (is_array($result) && isset($result['Value']) && $result['Value'] > 0);
            
                } catch (Exception $e) {
                    $errorMessage = 'Error: ' . $e->getMessage();
                    if ($testCall) {
                        return "SMS Gateway Error: " . $e->getMessage();
                    }
                    return false;
                }
        case 17:
...
$url = str_replace(array('{to}', '{sender_id}', '{message}', '{otp}', '{OTP}'), array($to, $sender_id, $messagetemplate,
$otp, $otp), $url);

if (sizeof($attrs) == 1) {
$attrs = implode(",", $attrs);
$data = str_replace(array('{to}', '{sender_id}', '{message}', '{otp}', '{OTP}'), array($to, $sender_id,
$messagetemplate, $otp, $otp), $attrs);
} else {
$data = array();
...
$attr_value = '';
if (isset($params[1]))
$attr_value = str_replace(array('{to}', '{sender_id}', '{message}', '{otp}', '{OTP}'), array($to, $sender_id,
$messagetemplate, $otp, $otp), $params[1]);
...
default:
return apply_filters('unitedover_send_sms', false, $option_slug, $gateway_id, $countrycode, $mobile, $messagetemplate,
$testCall);
}
